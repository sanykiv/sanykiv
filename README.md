...
BTC Market Analysis — проверка и опровержение гипотез 🔗 Live-демо (Streamlit) 
- Бэктест на 9-летней истории BTC/USDT. Из 3 гипотез строгую проверку (бенчмарк vs дрейф рынка, Bonferroni, Манна–Уитни, аудит look-ahead) пережила только одна — объём при пробоях (фильтр ложных пробоев 33% → 12%). Отскок от поддержки сам разоблачил как дрейф рынка. Метод перенесён на on-chain данные Uniswap v3. Python · pandas · SciPy · ccxt · Streamlit
